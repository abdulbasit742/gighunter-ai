...
function parseJsonFeed(data, sourceName) {
  ...
  return arr.map((j) => ({
    id: hash(String(j.id || j.url || j.slug || j.position || j.title || Math.random())),
    title: j.position || j.title || 'Untitled',
    company: j.company || j.company_name || j.companyName || '',
    url: j.url || j.apply_url || j.jobUrl || '',
    description: stripTags(j.description || j.excerpt || '').slice(0, 1200),
    budgetUsd: j.salary_min ? Number(j.salary_min) : null,
    source: sourceName,
    postedAt: j.date || j.pub_date || j.created_at || null,
  }));
}
...
export async function fetchSource(source) {
  try {
    ...
    if (source.type === 'json') return parseJsonFeed(await res.json(), source.name);
...
export async function fetchAll(sources = []) {
  ...
  // exact-id dedupe first
  const seen = new Map();
  for (const g of flat) if (!seen.has(g.id)) seen.set(g.id, g);
  // then near-duplicate collapse across platforms
